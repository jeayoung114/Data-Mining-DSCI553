...
object task2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val n = 3 // partition_num
    //    val support = 50
    //    val Filter_Threshold = 20
    //    val input_file = "/Users/jaeyoungkim/Desktop/usc/DSCI - 553/hw/hw2/ta_feng_all_months_merged.csv"
    //    val output_file = "tafeng_output.txt"
    ...
    val tmp = textRDD.filter(x => x != header)
      .map(x => x.split(","))
      .map(x => List(x(0).substring(0, x(0).length - 4).concat(x(0).substring(x(0).length - 2, x(0).length)),x(1), x(5)))
      .map(x => List(x(0).substring(1, x(0).size - 1),x(1).substring(1,x(1).size-1), x(2).substring(1, x(2).size - 1).toLong.toString))
      .map(x => List(x(0).concat("-").concat(x(1).toLong.toString),x(2)))

    val baskets = tmp.map(x => (x(0), List(x(1))))
      .reduceByKey(_ ++ _)
      .partitionBy(new HashPartitioner(n)).mapValues(x => x.toSet)
      .map(x => x._2)
      .filter(x => x.size>Filter_Threshold)
    ...
    def apriori(partition : Iterator[Set[String]], support: Int, entire_size : Int): List[List[String]] = {
      ...
      //Pair
      var pair_list = singleton.toList.combinations(2).toList.map(x => x.sorted)
    ...
    val intermediate = baskets.mapPartitions(x => apriori(x, support, entire_size).iterator)
      .map(x => Tuple2(x, 1))
      .reduceByKey((a,b) => a)

    val candidate_itemsets : List[List[String]] = intermediate.collect()
      .toList.sortBy(r => (r._1.length, r._1.mkString(" ") ))
      .map(x => x._1)
